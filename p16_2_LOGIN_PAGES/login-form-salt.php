...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($user) {
            // Hash the input password using SHA256 and the salt from the database
            $salt = $user['salt'];
            $input_hash = hash('sha256', $password . $salt);

            // Compare the hashed password
            if ($input_hash === $user['password_sha256']) {
                $_SESSION['user'] = $user;  
...
<html lang="en">
<head>
    <title>Login (SHA256 version with salt)</title>
    <meta charset="utf-8">
...
<main class="m-4 flex items-center justify-center h-screen">
    <section class="flex flex-col w-full max-w-md px-4 py-8 bg-white rounded-lg shadow dark:bg-gray-800 sm:px-6 md:px-8 lg:px-10">
        <div class="self-center mb-6 text-xl font-light text-gray-600 sm:text-2xl dark:text-white">
            Login (SHA256 version with salt)
        </div>
        <form method="post" action="login-form-salt.php" autocomplete="off">
        ...
        <!-- changed the link to what type of registration -->
